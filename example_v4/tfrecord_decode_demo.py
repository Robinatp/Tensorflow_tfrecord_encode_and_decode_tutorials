...
def decode_from_tfrecords(filename,num_epoch=None):
    ...
    image=tf.reshape(image,tf.stack([
        ...
        tf.cast(example['image/height'], tf.int32),3]))
...
#     img,l = decode_from_tfrecords('output/pet_val_with_masks.record')
    record_iterator = tf.python_io.tf_record_iterator(path='output/pet_val_with_masks.record')
