...
TF_API="/home/zhangbin/TensorFlow/master/models/research/object_detection"
...
def test():
    ...
    record_iterator = tf.python_io.tf_record_iterator(path='output/pet_val.record')
    ...
    with tf.Session() as sess:
        ...
        for string_iterator in record_iterator:
            ...
            encoded_mask_string = example.features.feature['image/object/mask'].bytes_list.value[0]
            
            plt.subplot(131)
            mask_placeholder = tf.placeholder(dtype=tf.string)
            mask_decode_png = tf.image.decode_png(mask_placeholder, channels=1)
            redecode_mask_img = sess.run(mask_decode_png, feed_dict={mask_placeholder: encoded_mask_string})
            ...
            vis_util.draw_mask_on_image_array(
                ...
                mask = np.squeeze(redecode_mask_img, axis = 2),
                alpha=0.8)
            plt.imshow(reconstructed_img)
